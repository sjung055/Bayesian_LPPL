...
# 2. data setting
## S&P 500 index data for diffuse priors: 1/3/1983 ~ 10/16/1987
diffuse_priors.data = snp_data[("1983-01-03" <= snp_data$Date) &
                             (snp_data$Date <= "1987-10-16"), ]
N = dim(diffuse_priors.data)[1]

## taking log to the price to make q(t)
### q(t_{i+1})
q_ti1 = log(diffuse_priors.data[2:N, ]$"Close")
### q(t_i)
q_ti = log(diffuse_priors.data[1:(N-1), ]$"Close")

## calendar time model
### t_{i+1}
ti1 = diffuse_priors.data[2:N, ]$"Date"
### t_i
ti = diffuse_priors.data[1:(N-1), ]$"Date"

### t
tN = diffuse_priors.data[N, ]$"Date"
...
for(t in 1:(iter_num-1)){
  ...
  if( (p.beta<0) | (1<p.beta) ){
    ...
    # 0 <= p.beta <= 1
    log_numerator = dunif(x = p.beta, min = beta.min,
                          max = beta.max, log = T) +
                    sum(log_probability_density(q_ti1, q_ti, ti1, ti,
                                                p.param, tc.t))
    log_denominator = dunif(x = beta.t, min = beta.min,
                            max = beta.max, log = T) +
                    sum(log_probability_density(q_ti1, q_ti, ti1, ti,
                                                param.t, tc.t))
    log_alpha = log_numerator - log_denominator
