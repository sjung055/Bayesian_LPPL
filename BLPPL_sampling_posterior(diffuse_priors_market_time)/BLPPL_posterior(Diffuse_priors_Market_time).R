...
# N+1 number of days
#   (t_0: diffuse_priors.data[1], t_2: diffuse_priors.data[2], ...,
#    t_N: diffuse_priors.data[N+1])
N1 = dim(diffuse_priors.data)[1]

## taking log to the price to make q(t)
### q(t_{i+1}): [q(t_1), q(t_2), ..., q(t_{N})]
q_ti1 = log(diffuse_priors.data[2:N1, ]$"Close")
### q(t_i): [q(t_0), q(t_1), ..., q(t_{N-1})]
q_ti = log(diffuse_priors.data[1:(N1-1), ]$"Close")
...
########################################################################
# 4. implementing "Gibbs sampler within MH algorithm"
start_time <- Sys.time()
for(t in 1:(iter_num-1)){
  ...
  if( (p.beta<0) | (1<p.beta) ){
    ...
    # 0 <= p.beta <= 1
    log_numerator = dunif(x = p.beta,
                          min = beta.min, max = beta.max, log = T) +
                    sum(log_probability_density(q_ti1, q_ti, ti1, ti,
                                                p.param, tc.t))
    log_denominator = dunif(x = beta.t, 
                            min = beta.min, max = beta.max, log = T) +
                    sum(log_probability_density(q_ti1, q_ti, ti1, ti,
                                                param.t, tc.t))
    log_alpha = log_numerator - log_denominator
